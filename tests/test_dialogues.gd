...
func test_invalid():
	...
	assert_dict(results).has_size(2)
	assert_int(results[5].type).is_equal(TypeChecker.TypeErrorType.UnknownMember)
	assert_int(results[6].type).is_equal(TypeChecker.TypeErrorType.UnknownMember)
